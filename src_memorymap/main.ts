...
import { textPatternStrings } from "../src/pattern";
...
let textPattern: boolean[][][];

function onLoad() {
  textPattern = text.setPattern(textPatternStrings);
  initColors();
  initCanvas();
  drawMemoryMap();
}
...
function print(text: string, x: number, y: number) {
  let tx = x;
